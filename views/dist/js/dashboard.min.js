...
angular.module('RDash').service('Usuario', function($rootScope, Http, EventHandler) {
...
angular.module('RDash').service('Categoria', function($rootScope, Http, EventHandler) {
    console.log('SERVICIO_CTGR');
    self = this;
...
angular.module('RDash').service('Elemento', function($rootScope, Http, EventHandler) {
...
angular.module('RDash').service('Costo', function($rootScope, Http, EventHandler) {
    console.log('JLPO_CREACION_SERVICIO_CST');
    self = this;
    ...
    self.get = function(costoId) {
        return Http.get('costos/' + costoId);
    };

    self.save = function(elemento) {
        return Http.post('costos/', elemento);
    };
...
/*CONTROLLERS*/
angular.module("RDash").controller('UsuariosCtrl', function($scope,  $location, Usuario) {
...
angular.module("RDash").controller('editUsuarioCtrl', function($scope, $location, Usuario, Departamento) {
    console.log('CONTROLADOR_EDIT_USR');

    refresh();

    function refresh() {
        $scope.usuario =idUsuarioActual;
        /*if(!(idUsuarioActual)){
            $location.path('/login');
        }else{
        }*/
...
angular.module("RDash").controller('CategoriasCtrl', function($scope,  $location, Categoria) {
...
angular.module("RDash").controller('editCategoriasCtrl', function($scope, $location, Categoria, Elemento) {
    ...
    function refresh() {
        $scope.categoria =idCategoriaActual;
        /*if(!(idUsuarioActual)){
            $location.path('/login');
        }else{*/
            if (idCategoriaActual){
                Categoria.getElementos(idCategoriaActual.id).then(function(categorias) {
    ...
    $scope.delElemento = function(ev, elemento) {
        Elemento.delete(elemento.id).then(function(){
            Categoria.getElementos(idCategoriaActual.id).then(function(categorias) {
                if (categorias.length > 0){
                    $scope.elementos = categorias[0].Elementos;
                }
...
angular.module("RDash").controller('editElementosCtrl', function($scope, $location, Elemento) {
    console.log('CONTROLADOR_EDIT_SB');

    refreshSb();

    function refreshSb() {
        $scope.elemento = idElementoActual;
        if (idElementoActual){
...
angular.module("RDash").controller('costosCtrl', function($scope, $location, Categoria, Costo) {
    ...
    function refreshSb() {
        ...
        Categoria.getAllElementos().then(function(elementos) {
            $scope.elementos = elementos;
        });
    }

    $scope.buscaCostos = function(){
        Costo.getCostos($scope.filtro.elmId).then(function(costos) {
            $scope.costos = costos;
        });
    };
